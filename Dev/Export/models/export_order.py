...
class ExportOrder(models.Model):
    ...
    shipping_c = fields.Char(string ='Đơn vị vận chuyển', required =True)
    ...
    cost = fields.Float(string='Chi phí vận chuyển', required = True)
    total_amount = fields.Float(string='Tổng thanh toán', compute='_compute_total_amount', store=True)
    cost_product =  fields.Float(string='Giá vốn', compute='_compute_cost_product', store=True)
    @api.depends('order_line.price_total')
    def _compute_total_amount(self):
        for order in self:
            order.total_amount = sum(line.price_total for line in order.order_line)
    @api.depends('order_line.cost_product')
    def _compute_cost_product(self):
        for order in self:
            order.cost_product = sum(line.cost_product for line in order.order_line)
class ExportOrderLine(models.Model):
    ...
    product_id = fields.Many2one('export.warehouse', string='Mã sản phẩm', required=True)
    ...
    def _compute_cost_product(self):
        for line in self:
            line.cost_product = line.quantity * (line.product_id.purchase_price)  # Giá nhập tính theo kg
    @api.model
    def create(self, vals):
        ...
        if product.quantity >= record.quantity:
            ...
            raise ValueError('Không đủ hàng trong kho: %s' % product._ame)
        return record
